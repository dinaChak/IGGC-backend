...
const studentObj = (branch, semester, subjects) => ({
  ...
  category: {
    type: ['General', 'ST', 'SC', 'OBC'][randomIntFromInterval(0, 3)],
    name: ''
  },
  nationality: 'India',
  bank_detail: {
    ...
    ac_no: faker.address.zipCode(),
    IFSC_code: faker.address.zipCode(),
  },
  presentAddress: {
  ...
  semester,
  subjectCombination: generatePapers(branch, semester, subjects),
...
const main = async () => {
  try {
    ...
    const students = Array(1000).fill(0).map(() => studentObj(['B.A', 'B.Com', 'B.Sc'][randomIntFromInterval(0, 2)], randomIntFromInterval(1, 5), subjects));
